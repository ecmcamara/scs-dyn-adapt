...
function ComponentContext:updateFacet(name, implementation)
  local facet = self._facetDescs[name]
...
function ComponentContext:removeFacet(name)
  if self._facetDescs[name] == nil then
    error("Facet does not exist.")
  end
  self:deactivateFacet(name)
  self._facetDescs[name] = nil
  self[name] = nil
...
function ComponentContext:addFacet( name, interface, implementation, key)
  if self._facetDescs[name] ~= nil then
    error("Facet already exists.")
  end
  self:putFacet( name, interface, implementation, key)
end

function ComponentContext:deactivateFacet( name)
  self._orb:deactivate(self._facetDescs[name].facet_ref)
end


function ComponentContext:putFacet( name, interface, implementation, key)
  ...
  self._facetDescs[name] = {name = name, interface_name = interface,
